...
exports.obtenerCarritos = async (req, res) => {
    try {
        ...
        res.status(500).json({mensaje: 'Error al obtener los carritos', error: error.message})
    }
};

exports.actualizarCarrito = async (req, res) => {
    try {
        const carritoActualizado = await Carrito.findByIdAndUpdate(req.params.id, req.body, {
            new: true
        });
